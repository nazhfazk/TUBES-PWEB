...
$siswa = new Siswa();
$guru = new model_guru();
$data = $siswa->getSiswaByNis($_GET);
foreach ($data as $item) {
    $nm_siswa = $item["nm_siswa"];
    $nis = $item["nis"];
    $nrp = $item["nrp"];
...
<body>
    <div class="container-fluid">
        <h2>Ubah Data</h2>
        <form action="model_siswa.php?ubah" method="POST">

            <div class="form-group">
                <input type="hidden" class="form-control" name="nis" value=<?= $_GET['nis'] ?>>
            </div>
            <?php
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" name="nama" value=<?= $nm_siswa ?>>
            </div>

            <div>
                <select class="custom-select custom-select-sm" name="nrp">
